...
@if ($currentTheme === 'bootstrap')
    @include('searchable-select::searchable-select-bootstrap')
@else
    @php
        $selectedValues =
            $multiple && is_array($selectedValue) ? $selectedValue : ($selectedValue ? [$selectedValue] : []); // Pre-build options array for Alpine.js
        $alpineOptions = [];
        if ($grouped) {
