<div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">Track supplier orders across the full procurement lifecycle.</p>
        </div>
    ...
    <div class="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-700/60 dark:bg-zinc-900">
        ...
        <div class="flex flex-wrap items-center gap-3 border-b border-zinc-200 bg-zinc-50/60 px-5 py-3 dark:border-zinc-700/60 dark:bg-zinc-800/40">
            ...
            <div class="flex gap-1.5">
                @foreach(['' => 'All', 'draft' => 'Draft', 'approved' => 'Approved', 'sent' => 'Sent', 'cancelled' => 'Cancelled'] as $val => $label)
                    @php
                        $active = $statusFilter === $val;
                        $base = 'rounded-md px-2.5 py-1 text-xs transition-colors ';
                        $colors = [
                            ...
                            'approved' => 'bg-violet-50 text-violet-700 ring-violet-200 hover:bg-violet-100 dark:bg-violet-900/20 dark:text-violet-400 dark:ring-violet-800/30',
                    ...
                    <button wire:click="$set('statusFilter', '{{ $val }}')"
                        class="{{ $base }}{{ $colors[$val] }} {{ $active ? 'ring-2 font-semibold' : 'ring-1' }}">
                        {{ $label }}
                    </button>
                @endforeach
            </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700/60 dark:bg-zinc-800/60">
                    ...
                    <th class="px-5 py-3 text-center text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Order</th>
                    <th class="px-5 py-3 text-center text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Receipt</th>
                    <th class="px-5 py-3 text-center text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Payment</th>
                ...
                @forelse($this->orders as $order)
                    <tr class="group transition-colors hover:bg-zinc-50/60 dark:hover:bg-zinc-800/30" wire:key="order-{{ $order->id }}">
                        ...
                        <td class="px-5 py-3.5 text-center">
                            <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold {{ $order->order_status->pillClasses() }}">
                                <span class="size-1.5 rounded-full {{ $order->order_status->dotClasses() }}"></span>
                                {{ $order->order_status->label() }}
                            </span>
                        </td>
                        <td class="px-5 py-3.5 text-center">
                            <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold {{ $order->receipt_status->pillClasses() }}">
                                <span class="size-1.5 rounded-full {{ $order->receipt_status->dotClasses() }}"></span>
                                {{ $order->receipt_status->label() }}
                            </span>
                        </td>
                        <td class="px-5 py-3.5 text-center">
                            <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold {{ $order->payment_status->pillClasses() }}">
                                <span class="size-1.5 rounded-full {{ $order->payment_status->dotClasses() }}"></span>
    ...
    {{-- Cancel Modal --}}
    <flux:modal name="cancel-order" class="max-w-md">
        <div class="space-y-5 p-6">
            <div>
                <h3 class="text-base font-semibold text-zinc-900 dark:text-zinc-100">Cancel Purchase Order</h3>
                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                    Depending on the order stage, additional steps may be required (Return to Supplier, Credit Note, or Refund).
                </p>
            </div>
            <div>
                <flux:label>Reason for cancellation <span class="text-red-500">*</span></flux:label>
                <flux:textarea wire:model="cancelReason" rows="3" placeholder="Explain why this order is being cancelled…" />
                <flux:error name="cancelReason" />
            </div>
            <div class="flex justify-end gap-3">
                <flux:button variant="ghost" x-on:click="$flux.modal('cancel-order').close()">Back</flux:button>
                <flux:button wire:click="cancelOrder" variant="danger">Confirm Cancellation</flux:button>
            </div>
        </div>
    </flux:modal>
