<div class="space-y-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <div class="flex items-center gap-2">
                    <h1 class="font-mono text-xl font-bold text-zinc-900 dark:text-zinc-100">
                        {{ $this->order->po_number }}</h1>
                    <span
                        class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold {{ $this->order->order_status->pillClasses() }}">
                        <span class="size-1.5 rounded-full {{ $this->order->order_status->dotClasses() }}"></span>
                        {{ $this->order->order_status->label() }}
                    </span>
                    <span
                        class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold {{ $this->order->closure_status->pillClasses() }}">
        ...
        @can('inventory.edit')
            <div class="flex flex-wrap gap-2">
                ...
                @if ($this->order->order_status === \App\Enums\OrderStatus::Approved)
                    ...
                    <flux:button wire:click="sendPo" variant="primary" icon="paper-airplane" size="sm">Send to Supplier
                    </flux:button>
                @endif
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
        @foreach ([['label' => 'Receipt', 'status' => $this->order->receipt_status->label(), 'pill' => $this->order->receipt_status->pillClasses(), 'dot' => $this->order->receipt_status->dotClasses()], ['label' => 'Invoice', 'status' => $this->order->invoice_status->label(), 'pill' => $this->order->invoice_status->pillClasses(), 'dot' => $this->order->invoice_status->dotClasses()], ['label' => 'Payment', 'status' => $this->order->payment_status->label(), 'pill' => $this->order->payment_status->pillClasses(), 'dot' => $this->order->payment_status->dotClasses()]] as $s)
            <div class="rounded-xl border border-zinc-200 bg-white px-4 py-3 dark:border-zinc-700/60 dark:bg-zinc-900">
                ...
                <span
                    class="mt-1 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold {{ $s['pill'] }}">
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="space-y-5 lg:col-span-2">
            ...
            {{-- Order Items --}}
            <div
                class="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-700/60 dark:bg-zinc-900">
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-zinc-100 bg-zinc-50/60 dark:border-zinc-700/50 dark:bg-zinc-800/40">
                            <th
                                class="px-5 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-zinc-500">
                                Item</th>
                            <th
                                class="px-5 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-zinc-500">
                                Ordered</th>
                            <th
                                class="px-5 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-zinc-500">
                                Received</th>
                            ...
                            <tr>
                                <td class="px-5 py-3 font-medium text-zinc-900 dark:text-zinc-100">
                                    ...
                                    <div class="mt-0.5 text-xs text-zinc-400 dark:text-zinc-500">
                                        {{ $item->variant_composition_label }}
                                    </div>
                                </td>
                                <td class="px-5 py-3 text-right tabular-nums text-zinc-600 dark:text-zinc-400">
                                    {{ $item->quantity }}</td>
                                <td
                                    class="px-5 py-3 text-right tabular-nums {{ $item->received_quantity < $item->quantity ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400' }}">
                                    {{ $item->received_quantity }}
                                </td>
                                <td
                                    class="px-5 py-3 text-right tabular-nums {{ $item->invoiced_quantity < $item->quantity ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400' }}">
                                    {{ $item->invoiced_quantity }}
                                </td>
                                <td class="px-5 py-3 text-right tabular-nums text-zinc-600 dark:text-zinc-400">UGX
                                    {{ number_format($item->unit_cost, 0) }}</td>
                                <td
                                    class="px-5 py-3 text-right font-semibold tabular-nums text-zinc-900 dark:text-zinc-100">
                                    UGX {{ number_format($item->subtotal, 0) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
                        <tr class="border-t border-zinc-200 bg-zinc-50/60 dark:border-zinc-700/60 dark:bg-zinc-800/40">
                            <td colspan="5"
                                class="px-5 py-3 text-right text-sm font-semibold text-zinc-900 dark:text-zinc-100">
                                Total</td>
                            <td class="px-5 py-3 text-right font-bold tabular-nums text-zinc-900 dark:text-zinc-100">UGX
                                {{ number_format($this->order->total_amount, 0) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
                    ...
                    @foreach ($this->order->goodsReceipts as $grn)
                        <div class="border-b border-zinc-100 px-5 py-4 last:border-0 dark:border-zinc-700/50">
                            <div class="flex items-center justify-between">
                                <div>
                                    <span
                                        class="font-mono text-sm font-semibold text-zinc-900 dark:text-zinc-100">{{ $grn->grn_number }}</span>
                                    <span
                                        class="ml-2 text-xs text-zinc-500 dark:text-zinc-400">{{ $grn->received_date->format('d M Y') }}</span>
                                    @if ($grn->receivedBy)
                                        <span class="ml-2 text-xs text-zinc-400 dark:text-zinc-500">by
                                            {{ $grn->receivedBy->name }}</span>
                                    @endif
                                </div>
                                <span class="text-xs text-zinc-400">{{ $grn->items->count() }}
                                    {{ Str::plural('line', $grn->items->count()) }}</span>
                            </div>
                            @if ($grn->notes)
                            ...
                            <div class="mt-2 space-y-1">
                                @foreach ($grn->items as $grnItem)
                                    <div class="flex items-center justify-between text-xs text-zinc-600 dark:text-zinc-400 px-2 py-1 rounded-md transition-colors duration-150 hover:bg-zinc-100 dark:hover:bg-zinc-800 cursor-pointer">
                                        <span>{{ $grnItem->inventoryItem->name }}</span>
                                        <span>
                                            {{ $grnItem->purchaseOrderItem?->variant_composition_label ?? '--' }}
                                        </span>
                    ...
                    @foreach ($this->order->supplierInvoices as $invoice)
                        <div class="border-b border-zinc-100 px-5 py-4 last:border-0 dark:border-zinc-700/50">
                            <div class="flex flex-wrap items-start justify-between gap-2">
                                <div>
                                    <span
                                        class="font-mono text-sm font-semibold text-zinc-900 dark:text-zinc-100">{{ $invoice->invoice_number }}</span>
                                    @if ($invoice->supplier_invoice_ref)
                                        <span class="ml-2 text-xs text-zinc-400">(Ref:
                                            {{ $invoice->supplier_invoice_ref }})</span>
                                    @endif
                                    <span
                                        class="ml-2 text-xs text-zinc-500 dark:text-zinc-400">{{ $invoice->invoice_date->format('d M Y') }}</span>
                                    @if ($invoice->due_date)
                                        <span
                                            class="ml-2 text-xs {{ $invoice->due_date->isPast() && $invoice->payment_status !== \App\Enums\PaymentStatus::FullyPaid ? 'text-red-500' : 'text-zinc-400' }}">
                                ...
                                <div class="flex items-center gap-3">
                                    <span
                                        class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold {{ $invoice->payment_status->pillClasses() }}">
                                        <span
                                            class="size-1.5 rounded-full {{ $invoice->payment_status->dotClasses() }}"></span>
                                        {{ $invoice->payment_status->label() }}
                                    </span>
                                    <span class="font-semibold tabular-nums text-zinc-900 dark:text-zinc-100">UGX
                                    ...
                                    @can('inventory.edit')
                                        @if ($invoice->payment_status !== \App\Enums\PaymentStatus::FullyPaid)
                                            <a href="{{ route('purchase-orders.payment', $invoice->id) }}" wire:navigate>
                                                <flux:button variant="ghost" icon="banknotes" size="xs">Pay
                                                </flux:button>
                                            </a>
                                        @endif
                                    @endcan
                            ...
                            @foreach ($invoice->payments as $pay)
                                <div
                                    class="mt-2 flex items-center justify-between rounded-lg bg-emerald-50 px-3 py-1.5 text-xs dark:bg-emerald-900/10">
                                    <div class="flex items-center gap-2 text-emerald-700 dark:text-emerald-400">
                                        ...
                                        <span class="text-zinc-400">&middot;
                                            {{ $pay->payment_date->format('d M Y') }}</span>
                                        <span
                                            class="capitalize text-zinc-400">{{ str_replace('_', ' ', $pay->payment_method) }}</span>
                                    </div>
                                    <span class="font-semibold tabular-nums text-emerald-700 dark:text-emerald-400">UGX
        ...
        <div class="space-y-5">
            ...
            {{-- Financial Summary --}}
            <div
                class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700/60 dark:bg-zinc-900">
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="font-semibold tabular-nums text-zinc-900 dark:text-zinc-100">UGX
                            {{ number_format($this->order->total_amount, 0) }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-zinc-500 dark:text-zinc-400">Total Invoiced</span>
                        <span class="font-semibold tabular-nums text-zinc-900 dark:text-zinc-100">UGX
                            {{ number_format($this->order->totalInvoiced(), 0) }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-zinc-500 dark:text-zinc-400">Total Paid</span>
                        <span class="font-semibold tabular-nums text-emerald-600 dark:text-emerald-400">UGX
                            {{ number_format($this->order->totalPaid(), 0) }}</span>
                    </div>
                    <div class="flex justify-between border-t border-zinc-100 pt-3 dark:border-zinc-700/60">
                        ...
                        <span class="font-bold tabular-nums text-red-600 dark:text-red-400">UGX
                            {{ number_format($this->order->outstandingBalance(), 0) }}</span>
                    </div>
                </div>
                @if ($this->order->expected_at)
                    <div
                        class="mt-4 rounded-lg bg-zinc-50 px-3 py-2 text-xs text-zinc-500 dark:bg-zinc-800/50 dark:text-zinc-400">
            ...
            @if ($this->order->order_status === \App\Enums\OrderStatus::Cancelled && $this->order->cancellation->isNotEmpty())
                ...
                <div class="rounded-xl border border-red-200 bg-red-50 p-5 dark:border-red-800/30 dark:bg-red-900/10">
                    ...
                    @if ($cancel->cancelled_at)
                        <p class="mt-1 text-xs text-zinc-400">{{ $this->order->cancelled_at?->format('d M Y, H:i') }}
                        </p>
                    @endif
                </div>
            @endif
