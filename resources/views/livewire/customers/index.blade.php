<div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">Customers</h1>
            <p class="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">Manage customer profiles and rental history.</p>
        </div>
    ...
    <div class="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-700/60 dark:bg-zinc-900">
        ...
        <div class="flex items-center justify-between gap-4 border-b border-zinc-200 bg-zinc-50/60 px-5 py-3 dark:border-zinc-700/60 dark:bg-zinc-800/40">
            ...
            <span class="text-xs font-medium text-zinc-400 dark:text-zinc-500">
                {{ $this->customers->total() }} {{ Str::plural('customer', $this->customers->total()) }}
            </span>
        </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700/60 dark:bg-zinc-800/60">
                    ...
                    <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">ID / Passport</th>
            ...
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-700/50">
                @forelse ($this->customers as $customer)
                    <tr class="group transition-colors hover:bg-zinc-50/60 dark:hover:bg-zinc-800/30" wire:key="customer-{{ $customer->id }}">
                        <td class="px-5 py-3.5">
                            <div class="flex items-center gap-3">
                                <div class="flex size-8 shrink-0 items-center justify-center rounded-full bg-amber-100 text-xs font-bold text-amber-700 dark:bg-amber-900/40 dark:text-amber-400">
                                    {{ strtoupper(substr($customer->name, 0, 1)) }}
                                </div>
                                <div>
                                    <div class="font-semibold text-zinc-900 dark:text-zinc-100">{{ $customer->name }}</div>
                                    @if($customer->address)
                                        <div class="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5 truncate max-w-xs">{{ $customer->address }}</div>
                                    @endif
                                </div>
                            </div>
                        </td>
                        <td class="px-5 py-3.5">
                            <div class="text-zinc-800 dark:text-zinc-200">{{ $customer->phone }}</div>
                            @if($customer->email)
                                <div class="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5">{{ $customer->email }}</div>
                            @endif
                        </td>
                        <td class="px-5 py-3.5">
                            @if($customer->id_number)
                                <code class="rounded bg-zinc-100 px-2 py-0.5 text-xs font-mono tracking-wide text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">
                                    {{ $customer->id_number }}
                                </code>
                            @else
                        ...
                        <td class="px-5 py-3.5 text-right">
                            <div class="flex items-center justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                                ...
                                <button wire:click="openDelete({{ $customer->id }})"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-5 py-16 text-center">
                            ...
                            @if(!$search)
                                <p class="mt-1 text-xs text-zinc-400 dark:text-zinc-500">Add your first customer to get started.</p>
                            @endif
                        </td>
                    </tr>
                @endforelse
    ...
    {{-- Customer Form Modal --}}
    <flux:modal name="customer-form" class="md:w-[36rem]">
        <form wire:submit="save" class="space-y-5">
            <div class="border-b border-zinc-100 pb-4 dark:border-zinc-700">
                ...
                <p class="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">
                    {{ $editingId ? 'Update the customer\'s details.' : 'Fill in the form to register a new customer.' }}
                </p>
            </div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div class="sm:col-span-2">
                    ...
                    <flux:input wire:model="name" placeholder="e.g. Jane Doe" />
                    <flux:error name="name" />
                </div>
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-zinc-700 dark:text-zinc-300">Phone <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-zinc-700 dark:text-zinc-300">ID / Passport</label>
                    <flux:input wire:model="idNumber" placeholder="e.g. CM123456" />
                    <flux:error name="idNumber" />
                </div>
                <div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <flux:textarea wire:model="notes" rows="2" placeholder="Any additional notes…" />
                </div>
            </div>

            <div class="flex justify-end gap-2 border-t border-zinc-100 pt-4 dark:border-zinc-700">
                <flux:button x-on:click="$flux.modal('customer-form').close()" variant="ghost" type="button">Cancel</flux:button>
                <button type="submit"
    ...
    <flux:modal name="confirm-delete-customer" class="md:w-96">
        <div class="space-y-5">
            <div class="flex items-start gap-4">
                ...
                <div>
                    <h3 class="text-base font-semibold text-zinc-900 dark:text-zinc-100">Delete Customer</h3>
                    <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                        Are you sure? This cannot be undone. Customers with active or confirmed bookings cannot be deleted.
                    </p>
                </div>
            </div>
            <div class="flex justify-end gap-2 border-t border-zinc-100 pt-4 dark:border-zinc-700">
                <flux:button x-on:click="$flux.modal('confirm-delete-customer').close()" variant="ghost" type="button">Cancel</flux:button>
